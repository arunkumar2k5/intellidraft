...
class FileUploaderApp(tk.Tk):
    def __init__(self, target_dir=TARGET_DIR, max_file_size=MAX_FILE_SIZE):
        ...
        self.chip_list = []  # stores IC chips for genAi processing
        self.circuit_name = None  # stores the circuit name internally
    ...
    def _show_circuit_name(self):
        ...
        loading_popup = self._create_loading_popup()
        
        def process_genai():
            try:
                # Run genAi with the stored chip list
                if self.chip_list:
                    name = genAi(self.chip_list)
                else:
                ...
                self.after(0, lambda: self._finish_circuit_name(loading_popup, name))
            except Exception:
                self.after(0, lambda: self._finish_circuit_name(loading_popup, "Unknown"))
        
        # Run genAi in a separate thread to keep UI responsive
        threading.Thread(target=process_genai, daemon=True).start()
    
    def _create_loading_popup(self):
        """Create a loading popup with 'Thinking...' message."""
    ...
    def _finish_circuit_name(self, loading_popup, name):
        """Close loading popup and show the circuit name result."""
        try:
        ...
        self._show_circuit_popup(name)
        
        # Remove the button after showing the popup
    ...
    def _show_circuit_popup(self, circuit_name):
        """Show a popup dialog with circuit name and Proceed/Enter Manually buttons."""
        popup = tk.Toplevel(self)
        popup.title("Circuit Name")
        ...
        name_frame = tk.Frame(popup, bg=self.preview_bg, relief="flat")
        ...
        circuit_label.pack(pady=15, padx=10)
        ...
        proceed_btn = tk.Button(
            button_frame,
            text="Proceed",
            command=lambda: self._on_proceed(popup, circuit_name),
            bg=self.button_color,
            fg=self.button_fg,
        ...
        manual_btn = tk.Button(
            button_frame,
            text="Enter Manually",
            command=lambda: self._on_enter_manually(popup),
            bg=self.bg_color,
            fg=self.button_fg,
            font=("Segoe UI", 11, "bold"),
            relief="flat",
            activebackground="#39275c",
            activeforeground=self.button_fg,
            padx=30,
            pady=10,
            width=15
    ...
    def _on_proceed(self, popup, circuit_name):
        """Handle Proceed button click."""
        # Store the circuit name internally without displaying it
        self.circuit_name = circuit_name
        popup.destroy()
    
    def _on_enter_manually(self, popup):
        """Handle Enter Manually button click."""
        popup.destroy()
        
        # Show input dialog
        input_popup = tk.Toplevel(self)
        input_popup.title("Enter Circuit Name")
        input_popup.geometry("450x200")
        input_popup.resizable(False, False)
        input_popup.configure(bg=self.card_color)
        ...
        title_label = tk.Label(
            input_popup,
            text="Enter Circuit Name Manually",
        ...
        # Entry field
        entry_frame = tk.Frame(input_popup, bg=self.card_color)
        ...
        name_entry.pack(fill="x", ipady=8, padx=5)
        name_entry.focus()
        
        # Submit button
        def submit_manual_name():
            manual_name = name_entry.get().strip()
            if manual_name:
                # Store the circuit name internally without displaying it
                self.circuit_name = manual_name
                input_popup.destroy()
            else:
                messagebox.showwarning("Empty Input", "Please enter a circuit name.")
        
        submit_btn = tk.Button(
            input_popup,
            text="Submit",
            command=submit_manual_name,
            bg=self.button_color,
            fg=self.button_fg,
            font=("Segoe UI", 11, "bold"),
            relief="flat",
        ...
        name_entry.bind("<Return>", lambda e: submit_manual_name())
        
        # Center popup on screen
        input_popup.update_idletasks()
        x = (input_popup.winfo_screenwidth() // 2) - (input_popup.winfo_width() // 2)
        y = (input_popup.winfo_screenheight() // 2) - (input_popup.winfo_height() // 2)
    ...
    def _extract_part_numbers_from_csv(self, filepath, col=0):
        ...
        try:
            with open(filepath, "r", encoding="utf-8", errors="ignore", newline="") as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    if col < len(row) and (col + 1) < len(row):
                        part_number = row[col].strip()
                        ref_designator = row[col + 1].strip()
                        if part_number:
            ...
            # Store chip list for later genAi processing when button is clicked
            ...
            # Run DigiKey search in background
            self.run_digikey_search(unique_parts)
            return unique_parts
